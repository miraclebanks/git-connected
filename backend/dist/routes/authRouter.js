...
router.get(
  ...
  function (req, res) {
    ...
    res.redirect(
      ''
        .concat(process.env.FRONTEND_DEV_URL, '/profile?id=')
        .concat(userDetails._id)
    );
  }
);
// --- Twitter ---
router.get(
  '/auth/twitter',
  auth_1.default,
  passport_1.default.authorize('twitter')
);
router.get(
...
router.get(
  '/auth/github',
  auth_1.default,
  passport_1.default.authorize('github', { scope: ['read:user'] })
);
router.get(
  '/auth/github/callback',
  auth_1.default,
  passport_1.default.authorize('github', {
    failureRedirect: '/',
    session: true,
  }),
  function (req, res) {
